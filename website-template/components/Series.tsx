...
import { Book, Series } from "../@types/sanity";
    ...
    <div className="flex-1 p-4 space-y-4 transition-all duration-150 ease-in-out bg-gray-800 rounded-md shadow-lg">
      <div className="flex items-center justify-between w-full">
        <span className="relative inline-flex items-center">
          ...
          {deemphasize && (
            <button onClick={() => updateModal(<HiddenSeriesInfoModal />)}>
              <svg
                className="absolute top-0 right-0 w-4 h-4 text-gray-300 transform hover:text-white translate-x-1/3 -translate-y-1/3"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
...
function HiddenSeriesInfoModal() {
  const { updateModal } = useContext(ModalContext);
    ...
    <div className="absolute inset-0 flex items-center justify-center px-2">
      <div className="relative z-50 flex flex-col w-full max-w-md overflow-hidden bg-white border border-gray-200 divide-y divide-gray-200 rounded-md shadow-2xl">
        ...
        <h1 className="p-4 font-light tracking-wider text-gray-900">
          This series does not meet your filter criteria, but one or more of its
          constituent books does.
        </h1>
      </div>
    </div>
  );
