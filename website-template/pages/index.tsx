...
export default withDefaultLayout(function IndexPage({
  homePage,
      ...
      <div className="px-4 pt-8">
        <div className="mx-auto space-y-8 max-w-7xl">
          {homePage.components?.map((component, index) => {
            if (isBio(component))
              return <BioView key={index} content={component.content ?? []} />;
            ...
            if (isCarousel(component))
              return <BookCarousel key={index} books={component.books ?? []} />;
          })}
        </div>
      </div>
    </div>
